...
#define VIRTIOGPU_VK_CMD_NOTIFY_RING UINT32_C(190)
#define VIRTIOGPU_VK_CMD_WRITE_RING_EXTRA UINT32_C(191)
...
static PVIRTIOGPURING virtioGpuR3FindRing(PVIRTIOGPU pThis, uint64_t uRingId)
{
    for (unsigned i = 0; i < RT_ELEMENTS(pThis->aRings); ++i)
        if (pThis->aRings[i].fUsed && pThis->aRings[i].uRingId == uRingId)
            return &pThis->aRings[i];
    return NULL;
}
...
static void virtioGpuR3AdvanceSubmittedShared(PVIRTIOGPU pThis,
                                               const uint32_t *paResourceIds,
                                               uint32_t cResources)
{
    /* vn_ring_get_layout() places head/tail/status at 0/64/128 bytes.  When
     * the DRM execbuffer carries only BO handles, the serialized control
     * packet is unavailable to the host, but these locations are still
...
static int virtioGpuR3VulkanPipelineBarrierBatch(PVIRTIOGPU pThis, PVIRTIOGPURESOURCE pRes,
                                                 const VIRTIOGPUPIPELINEBARRIERCMD *paBarrier,
                                                 uint32_t cBarrier)
{
    if (!pThis || !pRes || !paBarrier || cBarrier < 2 || cBarrier > 64 || !pRes->fVulkanImage
        || !pThis->fVulkanQueue || !pThis->fVulkanSubmit)
        return VERR_INVALID_PARAMETER;
    VkImageMemoryBarrier aHostBarriers[64];
    RT_ZERO(aHostBarriers);
    uint32_t enmOldLayout = UINT32_MAX;
    uint32_t enmNewLayout = UINT32_MAX;
    VkPipelineStageFlags fSrcStage = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
    VkPipelineStageFlags fDstStage = VK_PIPELINE_STAGE_TRANSFER_BIT;
    uint32_t fDependency = 0;
    for (uint32_t i = 0; i < cBarrier; ++i)
    {
        VIRTIOGPUPIPELINEBARRIERCMD const *pBarrier = &paBarrier[i];
        if (!pBarrier->pbImageBarrier || pBarrier->cMemoryBarriers || pBarrier->cBufferBarriers
            || pBarrier->cImageBarriers != 1 || (i && pBarrier->fModern != paBarrier[0].fModern))
            return VERR_INVALID_PARAMETER;
        uint32_t sType = 0, fSrcAccess = 0, fDstAccess = 0, oldLayout = 0, newLayout = 0;
        uint32_t srcQueue = 0, dstQueue = 0, auRange[5];
        uint64_t cbNext = 0, uImage = 0;
        uint32_t const offAccessSrc = pBarrier->fModern ? 20 : 12;
        uint32_t const offAccessDst = pBarrier->fModern ? 36 : 16;
        uint32_t const offOldLayout = pBarrier->fModern ? 44 : 20;
        uint32_t const offNewLayout = pBarrier->fModern ? 48 : 24;
        ...
        memcpy(&fSrcAccess, pBarrier->pbImageBarrier + offAccessSrc, sizeof(fSrcAccess));
        memcpy(&fDstAccess, pBarrier->pbImageBarrier + offAccessDst, sizeof(fDstAccess));
        memcpy(&oldLayout, pBarrier->pbImageBarrier + offOldLayout, sizeof(oldLayout));
        memcpy(&newLayout, pBarrier->pbImageBarrier + offNewLayout, sizeof(newLayout));
        memcpy(&srcQueue, pBarrier->pbImageBarrier + offSrcQueue, sizeof(srcQueue));
        memcpy(&dstQueue, pBarrier->pbImageBarrier + offDstQueue, sizeof(dstQueue));
        memcpy(&uImage, pBarrier->pbImageBarrier + offImage, sizeof(uImage));
        memcpy(auRange, pBarrier->pbImageBarrier + offRange, sizeof(auRange));
        if (sType != uExpectedSType || cbNext || uImage == 0 || uImage > UINT32_MAX
            || (uint32_t)uImage != pRes->uResourceId || srcQueue != VK_QUEUE_FAMILY_IGNORED
            || dstQueue != VK_QUEUE_FAMILY_IGNORED || auRange[0] != VK_IMAGE_ASPECT_COLOR_BIT
            || auRange[1] || auRange[2] != 1 || auRange[3] || auRange[4] != 1
            || (pRes->enmVkImageLayout != VK_IMAGE_LAYOUT_UNDEFINED
                && oldLayout != (uint32_t)pRes->enmVkImageLayout)
            || (newLayout != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
                && newLayout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL))
            return VERR_INVALID_PARAMETER;
        if (!i)
        {
            enmOldLayout = oldLayout;
            enmNewLayout = newLayout;
            fSrcStage = pBarrier->fSrcStage ? pBarrier->fSrcStage : VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
            fDstStage = pBarrier->fDstStage ? pBarrier->fDstStage : VK_PIPELINE_STAGE_TRANSFER_BIT;
            fDependency = pBarrier->fDependency;
        }
        else if (oldLayout != enmOldLayout || newLayout != enmNewLayout
                 || pBarrier->fSrcStage != paBarrier[0].fSrcStage
                 || pBarrier->fDstStage != paBarrier[0].fDstStage
                 || pBarrier->fDependency != fDependency)
            return VERR_INVALID_PARAMETER;
        aHostBarriers[i] = { VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, NULL,
                             fSrcAccess, fDstAccess, (VkImageLayout)oldLayout,
                             (VkImageLayout)newLayout, VK_QUEUE_FAMILY_IGNORED,
                             VK_QUEUE_FAMILY_IGNORED, pRes->hVkImage,
                             { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 } };
    }
    PFN_vkGetDeviceProcAddr pfnGetDeviceProcAddr =
    ...
    if (!pfnGetDeviceProcAddr)
        return VERR_NOT_FOUND;
# define VK_BARRIER_BATCH_PROC(type, name) (type)pfnGetDeviceProcAddr(pThis->hVkDevice, name)
    PFN_vkResetCommandBuffer pfnResetCommandBuffer = VK_BARRIER_BATCH_PROC(PFN_vkResetCommandBuffer, "vkResetCommandBuffer");
    PFN_vkBeginCommandBuffer pfnBeginCommandBuffer = VK_BARRIER_BATCH_PROC(PFN_vkBeginCommandBuffer, "vkBeginCommandBuffer");
    PFN_vkEndCommandBuffer pfnEndCommandBuffer = VK_BARRIER_BATCH_PROC(PFN_vkEndCommandBuffer, "vkEndCommandBuffer");
    PFN_vkCmdPipelineBarrier pfnCmdPipelineBarrier = VK_BARRIER_BATCH_PROC(PFN_vkCmdPipelineBarrier, "vkCmdPipelineBarrier");
    PFN_vkResetFences pfnResetFences = VK_BARRIER_BATCH_PROC(PFN_vkResetFences, "vkResetFences");
    PFN_vkQueueSubmit pfnQueueSubmit = VK_BARRIER_BATCH_PROC(PFN_vkQueueSubmit, "vkQueueSubmit");
    PFN_vkWaitForFences pfnWaitForFences = VK_BARRIER_BATCH_PROC(PFN_vkWaitForFences, "vkWaitForFences");
    if (!pfnResetCommandBuffer || !pfnBeginCommandBuffer || !pfnEndCommandBuffer || !pfnCmdPipelineBarrier
        || !pfnResetFences || !pfnQueueSubmit || !pfnWaitForFences)
        return VERR_NOT_FOUND;
    VkCommandBuffer hCmd = pThis->hVkSubmitCommandBuffer;
    ...
    if (pfnResetFences(pThis->hVkDevice, 1, &hFence) != VK_SUCCESS
        || pfnResetCommandBuffer(hCmd, 0) != VK_SUCCESS || pfnBeginCommandBuffer(hCmd, &BeginInfo) != VK_SUCCESS)
        return VERR_NOT_SUPPORTED;
    pfnCmdPipelineBarrier(hCmd, fSrcStage, fDstStage, fDependency, 0, NULL, 0, NULL, cBarrier, aHostBarriers);
    if (pfnEndCommandBuffer(hCmd) != VK_SUCCESS || pfnQueueSubmit(pThis->hVkQueue, 1, &SubmitInfo, hFence) != VK_SUCCESS
        || pfnWaitForFences(pThis->hVkDevice, 1, &hFence, VK_TRUE, UINT64_C(1000000000)) != VK_SUCCESS)
        return VERR_NOT_SUPPORTED;
    pRes->enmVkImageLayout = (VkImageLayout)enmNewLayout;
    return VINF_SUCCESS;
# undef VK_BARRIER_BATCH_PROC
}
...
static int virtioGpuR3VulkanCopyImageBatch(PVIRTIOGPU pThis, PVIRTIOGPURESOURCE pSrc,
                                           PVIRTIOGPURESOURCE pDst, const VIRTIOGPUCOPYIMAGECMD *paCopy,
                                           uint32_t cCopy)
{
    if (!pSrc || !pDst || !paCopy || !cCopy || cCopy > 64 || !pSrc->fVulkanImage || !pDst->fVulkanImage
        || !pThis->fVulkanQueue || !pThis->fVulkanSubmit)
        return VERR_INVALID_PARAMETER;
    VkImageCopy aRegions[64];
    RT_ZERO(aRegions);
    for (uint32_t i = 0; i < cCopy; ++i)
    {
        const VIRTIOGPUCOPYIMAGECMD *pCopy = &paCopy[i];
        if (pCopy->uSrcImage != paCopy[0].uSrcImage || pCopy->uDstImage != paCopy[0].uDstImage
            || pCopy->enmSrcImageLayout != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
            || pCopy->enmDstImageLayout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL || pCopy->cRegions != 1
            || !pCopy->pbRegions)
            return VERR_INVALID_PARAMETER;
        uint32_t auSrcSubresource[4], auDstSubresource[4], auExtent[3];
        int32_t aiSrcOffset[3], aiDstOffset[3];
        uint32_t const offFields = pCopy->cbRegionStride == 80 ? 12 : 0;
        uint32_t const cbStride = pCopy->cbRegionStride ? pCopy->cbRegionStride : 68;
        if (cbStride < 68 || pCopy->cbRegions != cbStride)
            return VERR_INVALID_PARAMETER;
        memcpy(auSrcSubresource, pCopy->pbRegions + offFields + 0, sizeof(auSrcSubresource));
        memcpy(aiSrcOffset, pCopy->pbRegions + offFields + 16, sizeof(aiSrcOffset));
        memcpy(auDstSubresource, pCopy->pbRegions + offFields + 28, sizeof(auDstSubresource));
        ...
        memcpy(auExtent, pCopy->pbRegions + offFields + 56, sizeof(auExtent));
        if (auSrcSubresource[0] != VK_IMAGE_ASPECT_COLOR_BIT || auSrcSubresource[1]
            || auSrcSubresource[2] || auSrcSubresource[3] != 1 || aiSrcOffset[0] || aiSrcOffset[1]
            || aiSrcOffset[2] || auDstSubresource[0] != VK_IMAGE_ASPECT_COLOR_BIT || auDstSubresource[1]
            || auDstSubresource[2] || auDstSubresource[3] != 1 || aiDstOffset[0] || aiDstOffset[1]
            || aiDstOffset[2] || auExtent[0] != pSrc->uWidth || auExtent[1] != pSrc->uHeight
            || auExtent[2] != 1 || pSrc->uWidth != pDst->uWidth || pSrc->uHeight != pDst->uHeight)
            return VERR_INVALID_PARAMETER;
        aRegions[i].srcSubresource = { auSrcSubresource[0], auSrcSubresource[1], auSrcSubresource[2], auSrcSubresource[3] };
        aRegions[i].srcOffset = { aiSrcOffset[0], aiSrcOffset[1], aiSrcOffset[2] };
        aRegions[i].dstSubresource = { auDstSubresource[0], auDstSubresource[1], auDstSubresource[2], auDstSubresource[3] };
        aRegions[i].dstOffset = { aiDstOffset[0], aiDstOffset[1], aiDstOffset[2] };
        aRegions[i].extent = { auExtent[0], auExtent[1], auExtent[2] };
    }
    PFN_vkGetDeviceProcAddr pfnGetDeviceProcAddr =
        (PFN_vkGetDeviceProcAddr)pThis->pfnVkGetInstanceProcAddr(pThis->hVkInstance, "vkGetDeviceProcAddr");
    if (!pfnGetDeviceProcAddr)
        return VERR_NOT_FOUND;
# define VK_COPY_IMAGE_BATCH_PROC(type, name) (type)pfnGetDeviceProcAddr(pThis->hVkDevice, name)
    PFN_vkResetCommandBuffer pfnResetCommandBuffer = VK_COPY_IMAGE_BATCH_PROC(PFN_vkResetCommandBuffer, "vkResetCommandBuffer");
    PFN_vkBeginCommandBuffer pfnBeginCommandBuffer = VK_COPY_IMAGE_BATCH_PROC(PFN_vkBeginCommandBuffer, "vkBeginCommandBuffer");
    PFN_vkEndCommandBuffer pfnEndCommandBuffer = VK_COPY_IMAGE_BATCH_PROC(PFN_vkEndCommandBuffer, "vkEndCommandBuffer");
    PFN_vkCmdPipelineBarrier pfnCmdPipelineBarrier = VK_COPY_IMAGE_BATCH_PROC(PFN_vkCmdPipelineBarrier, "vkCmdPipelineBarrier");
    PFN_vkCmdCopyImage pfnCmdCopyImage = VK_COPY_IMAGE_BATCH_PROC(PFN_vkCmdCopyImage, "vkCmdCopyImage");
    PFN_vkResetFences pfnResetFences = VK_COPY_IMAGE_BATCH_PROC(PFN_vkResetFences, "vkResetFences");
    PFN_vkQueueSubmit pfnQueueSubmit = VK_COPY_IMAGE_BATCH_PROC(PFN_vkQueueSubmit, "vkQueueSubmit");
    PFN_vkWaitForFences pfnWaitForFences = VK_COPY_IMAGE_BATCH_PROC(PFN_vkWaitForFences, "vkWaitForFences");
    if (!pfnResetCommandBuffer || !pfnBeginCommandBuffer || !pfnEndCommandBuffer || !pfnCmdPipelineBarrier
        || !pfnCmdCopyImage || !pfnResetFences || !pfnQueueSubmit || !pfnWaitForFences)
        return VERR_NOT_FOUND;
    VkCommandBuffer hCmd = pThis->hVkSubmitCommandBuffer;
    ...
    pfnCmdPipelineBarrier(hCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0,
                          0, NULL, 0, NULL, 2, aBarriers);
    pfnCmdCopyImage(hCmd, pSrc->hVkImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                    pDst->hVkImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, cCopy, aRegions);
    if (pfnEndCommandBuffer(hCmd) != VK_SUCCESS || pfnQueueSubmit(pThis->hVkQueue, 1, &SubmitInfo, hFence) != VK_SUCCESS
        || pfnWaitForFences(pThis->hVkDevice, 1, &hFence, VK_TRUE, UINT64_C(1000000000)) != VK_SUCCESS)
        return VERR_NOT_SUPPORTED;
    pDst->enmVkImageLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    pDst->fVulkanImageDirty = true;
    return VINF_SUCCESS;
# undef VK_COPY_IMAGE_BATCH_PROC
}
...
static int virtioGpuR3VulkanBlitImageBatch(PVIRTIOGPU pThis, PVIRTIOGPURESOURCE pSrc,
                                           PVIRTIOGPURESOURCE pDst, const VIRTIOGPUBLITIMAGECMD *paBlit,
                                           uint32_t cBlit)
{
    if (!pSrc || !pDst || !paBlit || !cBlit || cBlit > 64 || !pSrc->fVulkanImage || !pDst->fVulkanImage
        || !pThis->fVulkanQueue || !pThis->fVulkanSubmit)
        return VERR_INVALID_PARAMETER;
    VkImageBlit aRegions[64];
    RT_ZERO(aRegions);
    for (uint32_t i = 0; i < cBlit; ++i)
    {
        const VIRTIOGPUBLITIMAGECMD *pBlit = &paBlit[i];
        if (pBlit->uSrcImage != paBlit[0].uSrcImage || pBlit->uDstImage != paBlit[0].uDstImage
            || pBlit->enmSrcImageLayout != VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
            || pBlit->enmDstImageLayout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL || pBlit->cRegions != 1
            || !pBlit->pbRegions || (pBlit->enmFilter != VK_FILTER_NEAREST && pBlit->enmFilter != VK_FILTER_LINEAR))
            return VERR_INVALID_PARAMETER;
        uint32_t auSrcSubresource[4], auDstSubresource[4];
        uint64_t cSrcOffsets = 0, cDstOffsets = 0;
        uint32_t const offFields = pBlit->cbRegionStride == 108 ? 12 : 0;
        uint32_t const cbStride = pBlit->cbRegionStride ? pBlit->cbRegionStride : 96;
        if (cbStride < 96 || pBlit->cbRegions != cbStride)
            return VERR_INVALID_PARAMETER;
        memcpy(auSrcSubresource, pBlit->pbRegions + offFields + 0, sizeof(auSrcSubresource));
        memcpy(&cSrcOffsets, pBlit->pbRegions + offFields + 16, sizeof(cSrcOffsets));
        memcpy(aRegions[i].srcOffsets, pBlit->pbRegions + offFields + 24, sizeof(aRegions[i].srcOffsets));
        memcpy(auDstSubresource, pBlit->pbRegions + offFields + 48, sizeof(auDstSubresource));
        memcpy(&cDstOffsets, pBlit->pbRegions + offFields + 64, sizeof(cDstOffsets));
        memcpy(aRegions[i].dstOffsets, pBlit->pbRegions + offFields + 72, sizeof(aRegions[i].dstOffsets));
        if (auSrcSubresource[0] != VK_IMAGE_ASPECT_COLOR_BIT || auSrcSubresource[1]
            || auSrcSubresource[2] || auSrcSubresource[3] != 1 || cSrcOffsets != 2
            || auDstSubresource[0] != VK_IMAGE_ASPECT_COLOR_BIT || auDstSubresource[1]
            || auDstSubresource[2] || auDstSubresource[3] != 1 || cDstOffsets != 2)
        ...
        {
            int32_t const *paSrc = &aRegions[i].srcOffsets[j].x;
            int32_t const *paDst = &aRegions[i].dstOffsets[j].x;
            for (unsigned k = 0; k < 3; ++k)
                if (paSrc[k] < 0 || paDst[k] < 0
                    || (uint32_t)paSrc[k] > (k == 0 ? pSrc->uWidth : k == 1 ? pSrc->uHeight : 1)
                    || (uint32_t)paDst[k] > (k == 0 ? pDst->uWidth : k == 1 ? pDst->uHeight : 1))
                    return VERR_INVALID_PARAMETER;
        }
    }
    PFN_vkGetDeviceProcAddr pfnGetDeviceProcAddr =
    ...
    if (!pfnGetDeviceProcAddr)
        return VERR_NOT_FOUND;
# define VK_BLIT_BATCH_PROC(type, name) (type)pfnGetDeviceProcAddr(pThis->hVkDevice, name)
    PFN_vkResetCommandBuffer pfnResetCommandBuffer = VK_BLIT_BATCH_PROC(PFN_vkResetCommandBuffer, "vkResetCommandBuffer");
    PFN_vkBeginCommandBuffer pfnBeginCommandBuffer = VK_BLIT_BATCH_PROC(PFN_vkBeginCommandBuffer, "vkBeginCommandBuffer");
    PFN_vkEndCommandBuffer pfnEndCommandBuffer = VK_BLIT_BATCH_PROC(PFN_vkEndCommandBuffer, "vkEndCommandBuffer");
    PFN_vkCmdPipelineBarrier pfnCmdPipelineBarrier = VK_BLIT_BATCH_PROC(PFN_vkCmdPipelineBarrier, "vkCmdPipelineBarrier");
    PFN_vkCmdBlitImage pfnCmdBlitImage = VK_BLIT_BATCH_PROC(PFN_vkCmdBlitImage, "vkCmdBlitImage");
    PFN_vkResetFences pfnResetFences = VK_BLIT_BATCH_PROC(PFN_vkResetFences, "vkResetFences");
    PFN_vkQueueSubmit pfnQueueSubmit = VK_BLIT_BATCH_PROC(PFN_vkQueueSubmit, "vkQueueSubmit");
    PFN_vkWaitForFences pfnWaitForFences = VK_BLIT_BATCH_PROC(PFN_vkWaitForFences, "vkWaitForFences");
    if (!pfnResetCommandBuffer || !pfnBeginCommandBuffer || !pfnEndCommandBuffer || !pfnCmdPipelineBarrier
        || !pfnCmdBlitImage || !pfnResetFences || !pfnQueueSubmit || !pfnWaitForFences)
        return VERR_NOT_FOUND;
    VkCommandBuffer hCmd = pThis->hVkSubmitCommandBuffer;
    ...
    pfnCmdPipelineBarrier(hCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0,
                          0, NULL, 0, NULL, 2, aBarriers);
    pfnCmdBlitImage(hCmd, pSrc->hVkImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                    pDst->hVkImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, cBlit, aRegions,
                    (VkFilter)paBlit[0].enmFilter);
    if (pfnEndCommandBuffer(hCmd) != VK_SUCCESS || pfnQueueSubmit(pThis->hVkQueue, 1, &SubmitInfo, hFence) != VK_SUCCESS
        || pfnWaitForFences(pThis->hVkDevice, 1, &hFence, VK_TRUE, UINT64_C(1000000000)) != VK_SUCCESS)
        return VERR_NOT_SUPPORTED;
    pDst->enmVkImageLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    ...
    if (RT_SUCCESS(rcReq) && !(Req.uFlags & ~VIRTIOGPU_FLAG_FENCE) && uQueue == VIRTIOGPU_QUEUE_CONTROL)
    {
        switch (Req.uType)
        {
            ...
            case VIRTIOGPU_CMD_CTX_CREATE:
            {
                ...
                if (!Req.uCtxId || virtioGpuR3FindContext(pThis, Req.uCtxId)
                    ...
                    || (Cmd.fInit != 0 && Cmd.fInit != VIRTIOGPU_CONTEXT_INIT_CAPSET
                        && Cmd.fInit != VIRTIOGPU_CONTEXT_INIT_CAPSET_VENUS))
                    Resp.Hdr.uType = VIRTIOGPU_RESP_ERR_INVALID_PARAMETER;
                else
                {
            ...
            case VIRTIOGPU_CMD_SUBMIT_3D:
            {
                ...
                if ((Req.uCtxId && !pCtx) || pBuf->cbPhysSend < sizeof(Cmd)
                    || RT_FAILURE(virtioGpuR3Read(pDevIns, pVirtio, pBuf, &Cmd, sizeof(Cmd)))
                ...
                else
                {
                    ...
                    if (Cmd.cbCommand)
                        for (uint32_t i = 0; RT_SUCCESS(rcReq) && i < Cmd.cResources; ++i)
                            if (pCtx && !virtioGpuR3ContextHasResource(pCtx, auResourceIds[i]))
                                rcReq = VERR_INVALID_PARAMETER;
                    ...
                    {
                        /* Some virtio-gpu DRM paths submit the ring BOs with
                    ...
                    else
                    {
                        ...
                        VIRTIOGPUPIPELINEBARRIERCMD aPipelineBarrierBatch[64];
                        VIRTIOGPUCOPYIMAGECMD CopyImage;
                        VIRTIOGPUCOPYIMAGECMD CopyImage2;
                        VIRTIOGPUCOPYIMAGECMD aCopyImageBatch[64];
                                 ...
                                 || virtioGpuR3DecodeCopyImageToBuffer(pbCommand, Cmd.cbCommand, &CopyImageToBuffer))
                        {
                            ...
                            else
                            {
                                ...
                                Resp.Hdr.uType = VIRTIOGPU_RESP_ERR_UNSPEC;
                                  ...
                                  && virtioGpuR3DecodePipelineBarrier(pbCommand, 128, &PipelineBarrier))
                                 || (Cmd.cbCommand >= 328 && Cmd.cbCommand % 164 == 0
                                     && Cmd.cbCommand / 164 <= RT_ELEMENTS(aPipelineBarrierBatch)
                                     && virtioGpuR3DecodePipelineBarrier2(pbCommand, 164, &PipelineBarrier2)))
                        {
                            bool const fModernBarrier = Cmd.cbCommand % 164 == 0;
                            uint32_t const cbOne = fModernBarrier ? 164 : 128;
                            uint32_t const cBarrier = Cmd.cbCommand / cbOne;
                            bool fBatchValid = cBarrier >= 2;
                            PVIRTIOGPURESOURCE pImageRes = NULL;
                            uint64_t uImage = 0;
                            for (uint32_t i = 0; fBatchValid && i < cBarrier; ++i)
                                fBatchValid = fModernBarrier
                                             ? virtioGpuR3DecodePipelineBarrier2(pbCommand + i * cbOne, cbOne,
                                                                                 &aPipelineBarrierBatch[i])
                                             : virtioGpuR3DecodePipelineBarrier(pbCommand + i * cbOne, cbOne,
                                                                                &aPipelineBarrierBatch[i]);
                            if (fBatchValid)
                            {
                                ...
                                {
                                    uint64_t uCurImage = 0;
                                    memcpy(&uCurImage, aPipelineBarrierBatch[i].pbImageBarrier
                                           + (fModernBarrier ? 60 : 36), sizeof(uCurImage));
                                    if (uCurImage != uImage)
                                        fBatchValid = false;
                                }
                            }
                            if (fBatchValid)
                                Resp.Hdr.uType = RT_SUCCESS(virtioGpuR3VulkanPipelineBarrierBatch(pThis, pImageRes,
                                                                                                   aPipelineBarrierBatch,
                                                                                                   cBarrier))
                                               ? VIRTIOGPU_RESP_OK_NODATA : VIRTIOGPU_RESP_ERR_UNSPEC;
                            else
                                Resp.Hdr.uType = VIRTIOGPU_RESP_ERR_UNSPEC;
                            ...
                            if (fCopyImageBatchValid)
                            {
                                ...
                                for (uint32_t i = 1; fCopyImageBatchValid && i < cCopyImage; ++i)
                                    if (aCopyImageBatch[i].uSrcImage != aCopyImageBatch[0].uSrcImage
                                        || aCopyImageBatch[i].uDstImage != aCopyImageBatch[0].uDstImage)
                                        fCopyImageBatchValid = false;
                            }
                            if (fCopyImageBatchValid)
                                Resp.Hdr.uType = RT_SUCCESS(virtioGpuR3VulkanCopyImageBatch(pThis, pSrcRes, pDstRes,
                                                                                              aCopyImageBatch, cCopyImage))
                                               ? VIRTIOGPU_RESP_OK_NODATA : VIRTIOGPU_RESP_ERR_UNSPEC;
                            else
                                Resp.Hdr.uType = VIRTIOGPU_RESP_ERR_UNSPEC;
                        }
                        ...
                        else if (Cmd.cbCommand >= 368 && Cmd.cbCommand % 184 == 0
                                 && Cmd.cbCommand / 184 <= RT_ELEMENTS(aBlitBatch))
                        {
                            uint32_t const cBlit = Cmd.cbCommand / 184;
                            bool fBlitBatchValid = cBlit >= 2;
                            PVIRTIOGPURESOURCE pSrcRes = NULL;
                            PVIRTIOGPURESOURCE pDstRes = NULL;
                            for (uint32_t i = 0; fBlitBatchValid && i < cBlit; ++i)
                                fBlitBatchValid = virtioGpuR3DecodeBlitImage2(pbCommand + i * 184, 184,
                                                                              &aBlitBatch[i]);
                            if (fBlitBatchValid)
                            {
                                ...
                                for (uint32_t i = 1; fBlitBatchValid && i < cBlit; ++i)
                                    if (aBlitBatch[i].uSrcImage != aBlitBatch[0].uSrcImage
                                        || aBlitBatch[i].uDstImage != aBlitBatch[0].uDstImage)
                                        fBlitBatchValid = false;
                            }
                            if (fBlitBatchValid)
                                Resp.Hdr.uType = RT_SUCCESS(virtioGpuR3VulkanBlitImageBatch(pThis, pSrcRes, pDstRes,
                                                                                             aBlitBatch, cBlit))
                                               ? VIRTIOGPU_RESP_OK_NODATA : VIRTIOGPU_RESP_ERR_UNSPEC;
                            else
                                Resp.Hdr.uType = VIRTIOGPU_RESP_ERR_UNSPEC;
